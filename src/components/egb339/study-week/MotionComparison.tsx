...
export default function MotionComparison() {
  ...
  return <section className="egb-week-learning-block" aria-labelledby={`${id}-heading`} data-week-motion>
    <h3 id={`${id}-heading`}>Samme endepunkter, to forskjellige baner</h3>
    <p>QUT s. 5 og 7: L₁ = 2, L₂ = 1.5, start (0.75, 0.5), mål (0.75, −0.5). Begge metodene bruker den positive IK-grenen. Vi undersøker posisjonsbanen, ikke en tidsprofil.</p>
    <div className="egb-week-two-column">
      <figure className="egb-week-technical-figure">
        <svg viewBox="0 0 500 490" role="img" aria-labelledby={`${id}-title ${id}-desc`}>
          <title id={`${id}-title`}>Kartesisk interpolasjon er rett, leddinterpolasjon bøyer ut mot høyre</title>
          <desc id={`${id}-desc`}>Et koordinatutsnitt med lik målestokk på begge akser. Fem sirkler viser kartesiske samples, og fem kvadrater viser leddinterpolerte samples. Valgt punkt er forstørret.</desc>
        ...
        <figcaption>Egen beregnet figur etter QUTs fem samples. Blå stiplet/sirkel = kartesisk. Lilla heltrukket/kvadrat = ledd. Utsnitt, ikke fullt arbeidsrom; 400 px per lengdeenhet på begge akser.</figcaption>
      </figure>
      <div className="egb-week-explorer-controls">
        <div className="egb-week-control-row">{([['both', 'Sammenlign'], ['cartesian', 'Kartesisk'], ['joint', 'Ledd']] as const).map(([value, label]) => <button key={value} type="button" className="egb-week-button" aria-pressed={mode === value} onClick={() => setMode(value)}>{label}</button>)}</div>
        <label>QUT-sample {step + 1} av 5<input type="range" min="0" max="4" step="1" value={step} aria-label="Velg ett av fem QUT-punkter" onChange={e => setStep(Number(e.target.value))} /></label>
        <MathText>{String.raw`\alpha=\frac{i}{n-1}=\frac{${step}}{4}=${step / 4}`}</MathText>
        <p className="egb-week-frame-a">Kartesisk: interpoler p, løs så IK.</p>
        ...
        <p className="egb-week-frame-b">Ledd: interpoler q, beregn så FK.</p>
        ...
        <p role="status" aria-live="polite">Ved sample {step + 1} er avstanden mellom metodene {f(Math.hypot(current.joint[0] - current.cartesian[0], current.joint[1] - current.cartesian[1]))} lengdeenheter.</p>
        <button type="button" className="egb-week-button" onClick={() => { setStep(0); setMode("both"); }}>Reset</button>
      </div>
    </div>
    <p className="egb-week-notice" data-state="warning">Rett endeeffektor-bane betyr ikke at alle lenkene går klar av hindringer. Kontroller hver lenke gjennom bevegelsen.</p>
    <p className="egb-pilot-source">QUT Week 7 tutorial og fasit. Eksisterende FK/IK-motor brukes direkte. RTB <code>jtraj</code> bruker en femtegradspolynomprofil og er derfor ikke en direkte erstatning for tutorialens lineære interpolasjon.</p>
  </section>;
}
